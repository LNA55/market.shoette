...
- **2026-09-06 — Largeur fluide des pages de run S2/S3/S4** (demande d'Elena, arbitrée par questionnaire : « tout large et fluide », exemple à suivre = pages parent). Les pages de run alignent leur colonne sur celle des pages parent (`site.css`) : **`.shell / --maxw : calc(50vw + 440px)`** au lieu de 1180 px fixes, et **suppression de tous les plafonds de largeur** (h1 `16ch`, byline `72-74ch`, purpose `60-64ch`, prose/notes `78ch`, Porter `920px`, 9-box `760px`) — une seule largeur pour tout le contenu, qui grandit avec l'écran. Appliqué aux runs S2-1 et S3-1 de `perso-ecommerce` (déployé, vérifié de 375 à 2200 px — frise sans collision, zéro débordement nouveau ; les runs figés d'apps-perte-de-poids restent en 1180 px) ; **acté dans les contrats S2/S3/S4** pour tous les runs futurs, dont le run 4 à venir. Historique : premier signalement sur la note S3 (« cadres fins avec grandes marges latérales ») corrigé d'abord par l'uniformisation interne (retrait des plafonds 78ch), puis élargi ici à la colonne fluide.

- **2026-09-06 — Anglais par défaut sur deux pages (provisoire) + traduction du marché `perso-ecommerce`.** Demande d'Elena : actualiser l'accueil EN, ouvrir `perso-ecommerce` en anglais, et servir ces deux pages **en anglais par défaut « for now »**. (1) **Traduction** : `en/perso-ecommerce/` = page marché + run S1-1 complet (prose, `RUN_DATA` et `data.json` anglais). Méthode retenue pour le JSON — **carte de traduction indexée par chemin** appliquée programmatiquement au `data.json` FR, ce qui garantit par construction structure, identifiants et **valeurs numériques identiques** (131 nombres vérifiés), au lieu de réécrire le fichier à la main ; les valeurs ordinales sont contrôlées contre les échelles traduites. Le lexique A2 étant déjà bilingue (`term`/`fr`/`en`), il n'est pas traduit. **Le run FR figé n'est pas modifié.** (2) **Accueil EN** actualisé aux 4 marchés ; `greeting-cards-us` et `micro-seed`, sans version EN, pointent vers le FR avec la mention « in French » plutôt qu'un lien silencieux ; la carte du run S4-2 manquante a été ajoutée à `en/apps-perte-de-poids/` pour que le compteur « 7 runs » soit exact. (3) **Langue par défaut** : redirection **302** (jamais 301 — retirable sans purge de cache) de `/` et `/perso-ecommerce/` vers `/en/…`, dans `site/.htaccess`. Le **français reste atteignable** : `?lang=fr` (porté par le lien FR du sélecteur sur ces pages, via `DEFAULT_EN` dans `sitefoot.js`) échappe la redirection et fait poser un **cookie fonctionnel `lang=fr`** par `siteheader.js`, qui la désactive ensuite sur tout le site ; `?lang=en` efface le cookie. **Revenir au français par défaut = supprimer le bloc du `.htaccess` ET l'entrée dans `DEFAULT_EN`.** ⚠ **Piège Apache constaté** : un motif de `RewriteCond` contenant une espace **doit être entre guillemets**, faute de quoi Apache renvoie **500 sur tout le site** ; toute règle `.htaccess` se teste désormais dans un **sous-répertoire jetable** déployé à part, jamais directement sur le fichier racine.

- **2026-06-15 — Bilingue FR/EN, Phase 1b (vague 1 : moteur + pages légères).** Architecture : **FR = URLs actuelles, EN = préfixe `/en/`** ; défaut FR ; langue déduite de l'URL ; **sticky par propagation d'URL** (les liens internes relatifs restent dans la langue, une page EN vivant sous `/en/`). Les 3 composants partagés deviennent **conscients de la langue** : `siteheader` (marque → racine de langue `L = base + (EN?"en/":"")`), `sitefoot` (libellés FR/EN, liens, **+ sélecteur FR/EN à droite du footer**), `sitepipeline` (noms d'étapes FR/EN ; liens relatifs donc déjà par langue). **Mécanisme `EN_AVAILABLE`** (dans `sitefoot.js`) : tant qu'une page n'a pas de version EN, son lien footer retombe sur le FR **et** le « EN » du toggle est grisé (`is-soon`) → **zéro 404** ; étendre cette liste à chaque page traduite. Le toggle calcule la racine via `new URL(base, location.href)` (correct en local `/site/` comme en prod `/`). **Convention typo appliquée** : glose EN en *italique* (« a *run* »). **Pages EN créées** : accueil, marché, how-it-works (`EN_AVAILABLE = ["", "apps-perte-de-poids/", "how-it-works/"]`) ; leurs liens vers des pages non encore traduites (runs, focus) sont en **interim FR relatif**, à basculer. `hreflang` alternate posés. Au passage, mention « (en cours de définition) » des étapes 3/4 retirée de how-it-works (FR+EN — désormais fausses). **Restant** : vague 2 = focus 1-4 (dont le gros `focus-step-2`) ; Phase 2 = les 5 runs en EN (+ lexique A2 affiché à l'identique, bilingue) ; Phase 3 = skills/contrats pour que tout futur run et tout changement de style soient produits dans les deux langues.

- **2026-06-15 — Header : « Accueil » retiré du fil d'Ariane + how-it-works devient le parent des pages d'étape** (Phase 1a du chantier bilingue). (1) Le logo menant déjà à l'accueil, le crumb ne montre plus « Accueil » : pages de profondeur 1 (accueil, marché, how-it-works) = **logo seul** ; le crumb « parent » est désormais un lien unique « ← [parent] » (runs → « ← Marché : X », pages d'étape → « ← How it works »). (2) Les pages d'étape passent **sous `/how-it-works/`** : `/focus-step-N/` → `/how-it-works/focus-step-N/` (et le process en profondeur 3), **redirections 301** en place (anciennes URLs + `focus-skill-N`), tous les liens internes réécrits (sitefoot « La méthode », run S2 vers les fiches frameworks, how-it-works vers ses enfants), chemins relatifs des pages déplacées corrigés, `siteheader.js` mis à jour. Vérifié en ligne (200 / 301 / assets / crumbs). Reste de la Phase 1 : composants multilingues + footer langue + miroir `/en/`.

- **2026-06-15 — Convention typographique pour gloser un terme** (ex. « run ») : **FR → guillemets français « … »** ; **EN (version bilingue à venir, aujourd'hui inactive) → italique** (`*term*`). À appliquer systématiquement quand la traduction sera relancée, pour ne pas reperdre la règle. Premier emploi : l'accueil définit « rapport daté à chaque cycle (dit « run ») ».
