...
- **`players[].is_mine`** *(optionnel — jamais écrit par la Skill 1)* : `true` = le projet d'Elena, ajouté par la Skill 3 (avec `added_by_run`). Le moteur 2b lui donne un marqueur distinctif (rayons radiaux, nom en gras, badge « ✺ mon projet », note de légende). Contrat d'enrichissement : `.claude/skills/position-my-product/references/s3-contracts.md`.
...
- **`last_updated`** : la date du run — **jamais réécrite, même par la Skill 3** (décision 2026-06-12) : l'enrichissement S3 ajoute une entrée `players[]`, un bloc `enrichments[]` et une mention datée à côté de la date, sans toucher au reste.
- **Stabilité des `id`** (acteurs et dimensions) : slugs ASCII stables d'un run à l'autre — la Skill 3 s'en sert aussi (re-run d'un même projet → même `id`, entrée mise à jour).
